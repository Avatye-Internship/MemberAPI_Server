...
const validateUser = (user) => {
  ...
  if (
    ...
    errors.push(
      `비밀번호를 ${minLength}자 이상 ${maxLength}자 이하로 입력해주세요`
    );
  }
...
exports.register_controller = async (req, res) => {
  try {
    ...
    if (errors.length) {
      return res
        .status(400)
        .json({ message: RESPONSE_MESSAGES.VALIDATION_ERROR, errors });
    }
    var param = {
    ...
    try {
      ...
      const [rows] = await conn.query(UserQuery.have_user_query, [
        user.user_id,
      ]);
      //중복 이메일 에러 처리(409)
      if (rows.length > 0) {
        return res
          .status(409)
          .json({ message: RESPONSE_MESSAGES.EMAIL_EXISTS });
      }
      //회원 등록
      ...
      return res
        .status(201)
        .json({ message: RESPONSE_MESSAGES.REGISTER_SUCCESS });
    } catch (err) {
      //내부 서버 에러 처리(500)
...
exports.user_update_controller = async (req, res) => {
  ...
  if (phone) {
    if (!phoneRegex.test(phone)) {
      return res
        .status(409)
        .json({ message: "올바른 전화번호를 입력해주세요 (010-1234-5678)" });
    } else {
...
exports.send_verification_code_controller = async (req, res) => {
  ...
  try {
    ...
    //이메일 인증코드 db에 저장
    conn.query(UserQuery.send_verification_code_query, [
      verificationCode,
      user_id,
    ]);
    var mailOptions = {
...
exports.verify_code_controller = async (req, res) => {
  ...
  try {
    ...
    //이메일 인증코드 검증
    [rows] = await conn.query(UserQuery.verify_code_query, [
      user_id,
    ...
    if (rows.length === 0) {
      res
        .status(401)
        .json({ message: "이메일 인증 코드가 일치하지 않습니다." });
    }
...
exports.email_verification_code_delete_controller = async (req, res) => {
  ...
  try {
    ...
    [results] = await conn.query(
      UserQuery.email_verification_code_delete_query,
      [user_id]
    );
    //삭제된 칼럼없는 경우
...
exports.password_change_controller = async (req, res) => {
  ...
  try {
    ...
    const [results] = await conn.query(UserQuery.change_password_query, [
      hashPassword,
      salt,
      user_id,
    ]);
    ...
    if (!newPassword || !validator.isLength(newPassword, { min: 8, max: 15 })) {
      return res
        .status(400)
        .json({ message: "비밀번호를 8자 이상 15자 이하로 입력해주세요" });
    }
...
exports.mypage_controller = async (req, res) => {
  ...
  try {
    ...
    if (rows.length === 0) {
      ...
      return res
        .status(401)
        .json({ success: false, errormessage: "조회 정보가 없습니다." });
    }
    console.log(rows);
...
exports.mypage_terms_controller = async (req, res) => {
  ...
  try {
    ...
    const [rows] = await conn.query(UserQuery.mypage_terms_select_query, [
      user_id,
    ]);
...
exports.point_controller = async (req, res) => {
  ...
  try {
    const conn = await pool.getConnection();
    //포인트 변경
    [results] = await conn.query(UserQuery.point_update_query, [
      point,
      user_id,
    ]);
    if (results.affectedRows === 0) {
    ...
    //등급 수정
    [results] = await conn.query(UserQuery.grade_update_query, [
      gradeId,
      user_id,
    ]);
    if (results.affectedRows === 0) {
...
exports.terms_update_controller = async (req, res) => {
  ...
  try {
    const conn = await pool.getConnection();
    const [rows] = await conn.query(UserQuery.terms_update_query, [
      agree_check,
      user_id,
    ]);
    console.log(rows);
